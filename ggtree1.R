...
genotype <- read.table('nodes_genotypes_vp7.txt',sep="\t", 
stringsAsFactor=F )
...
gheatmap(p, genotype, offset = 0.4, width=0.4, 
...
ggtree(rax) + geom_nodelab(aes(label = label, 
subset = as.numeric(label) > 70))

#reroot tree to G6
rax_rooted <- rax
rax_rooted@phylo <- root(rax_rooted@phylo, 
outgroup = "RVA/Roe-deer-wt/SLO/D38-14/2014/G6P15")
rax_rooted <- reroot(as.phylo(rax_rooted), outgroup)

p1 <- ggtree(rax) + 
      geom_treescale(y=-2) + 
      geom_nodelab(aes(label=label,subset=as.numeric(label) > 70), size=3) +
      geom_tiplab(align = T, size = 4)

gheatmap(p1, genotype, offset = 0.4, width=0.4, 
        colnames_position = "top", font.size = 3) +
        scale_fill_manual(values=cols)